...
/*   Updated: 2023/07/22 22:09:14 by rrhnizar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_keys(int keycode, t_mlx *mlx_cub)
{
	...
	{
		// Move left perpendicular to the view direction
		tmp_xp = mlx_cub->player->x + (((int)(mlx_cub->pdx * cos(-M_PI / 2) - mlx_cub->pdy * sin(-M_PI / 2))) / 10);
		tmp_yp = mlx_cub->player->y + (((int)(mlx_cub->pdx * sin(-M_PI / 2) + mlx_cub->pdy * cos(-M_PI / 2))) / 10);
	
		if (tmp_yp >= 0 && tmp_yp <= mlx_cub->h && tmp_xp >= 0 && tmp_xp <= mlx_cub->w)
		{
			if (mlx_cub->ele->map[(tmp_yp) / 50][(tmp_xp) / 50] != '1')
			{
				mlx_cub->x_p_move += (mlx_cub->pdx * cos(-M_PI / 2) - mlx_cub->pdy * sin(-M_PI / 2)) / 10;
				mlx_cub->y_p_move += (mlx_cub->pdx * sin(-M_PI / 2) + mlx_cub->pdy * cos(-M_PI / 2)) / 10;
			}
		}
	}
	else if (keycode == D)
	{
		// Move right perpendicular to the view direction
		tmp_xp = mlx_cub->player->x + (((int)(mlx_cub->pdx * cos(M_PI / 2) - mlx_cub->pdy * sin(M_PI / 2))) / 10);
		tmp_yp = mlx_cub->player->y + (((int)(mlx_cub->pdx * sin(M_PI / 2) + mlx_cub->pdy * cos(M_PI / 2))) / 10);
